...
export default function PricingPage() {
  ...
  const handleUpgrade = async (tierName: string) => {
    ...
    // Paid tiers now handled by RazorpayCheckoutButton directly
  };

  /**
   * Amount in paise (1 INR = 100 paise).
   * Roughly: $9.99 ≈ ₹830, $19.99 ≈ ₹1659 at ~₹83/USD.
   * Adjust prices to your preferred INR rates.
   */
  const tierAmountPaise: Record<string, number> = {
    Pro: billingCycle === "monthly" ? 83000 : 79900,    // ₹830 / ₹799
    Premium: billingCycle === "monthly" ? 166500 : 132900, // ₹1665 / ₹1329
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto items-stretch">
            {tiers.map((tier, index) => (
              ...
              >
                ...
                {/* CTA Button */}
                {tier.name === "Free" || currentTier === tier.name.toUpperCase() ? (
                  <Button
                  ...
                  <RazorpayCheckoutButton
                    amount={tierAmountPaise[tier.name] ?? 83000}
                    currency="INR"
                    description={`Wander.AI ${tier.name} Plan — ${billingCycle === "monthly" ? "Monthly" : "Yearly"}`}
                    buttonLabel={session ? tier.cta : "Sign in to Upgrade"}
                    prefillEmail={session?.user?.email ?? undefined}
                    prefillName={session?.user?.name ?? undefined}
                    disabled={!session}
                    notes={{ tier: tier.name, billingCycle }}
                    className="w-full rounded-full py-2.5 text-xs"
                    onSuccess={(paymentId, orderId) => {
                      console.info("Payment verified:", { paymentId, orderId, tier: tier.name });
                      router.push("/dashboard?payment=success&tier=" + tier.name.toLowerCase());
                    }}
                    onFailure={(err) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {[
              ...
              },
              {
                ...
                a: "We accept UPI (GPay, PhonePe, Paytm), all major credit & debit cards, net banking, and wallets through Razorpay's PCI-DSS compliant checkout.",
              },
              {
