...
import { ItineraryCard } from "@/app/components/ui/ItineraryCard";
...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    fetchItineraries();
  }, [filter, debouncedSearchQuery, page]);

  const fetchItineraries = async () => {
    setLoading(true);
    try {
      const response = await fetch(
        `/api/marketplace?filter=${filter}&search=${encodeURIComponent(debouncedSearchQuery)}&page=${page}`
      );
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-8 pb-16">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-semibold mb-4">
            ...
            <span>World of Travellers</span>
          </div>
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight text-foreground mb-3 font-serif">
          ...
          {/* Quick Chat Room Banner & Actions */}
          <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            <Button
              asChild
              variant="outline"
              className="rounded-full px-5 py-2 text-xs font-bold border-border/80 hover:border-primary/40 bg-card hover:bg-muted/60 text-foreground transition-all shadow-2xs gap-2"
            >
